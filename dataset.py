...
class SER():
    ...
    def __getitem__(self, index):
        ...
        return img, target# , topic_id, topic_mask # , imgname

    def __len__(self):
        return len(self.annos)

def detect_mg_patterns(text, n=4):

    # 仅匹配四个关键类别，作为分割点
    pattern = r'(#The sticker\'s (?:intention|style|main role|gesture))'
...
class SER_Full():
    def __init__(self, root, mode='train', data_len=None, transform=None, max_query_len=512, bert_model='bert-base-uncased'):
        self.root = "../dataset"
        self.mode = mode
        self.nb_classes = 7
        self.transform = transform
        ...
        with open(join(self.root, 'Annotations_mg', 'image-level', f'{self.mode}.json'), 'r') as f:
            annos = json.load(f)
        self.annos = annos['annotations']
        self.imgs = [os.path.join(self.root, 'Images', anno['topic'], anno['file_name']) for anno in
                            tqdm(self.annos[:data_len])]
    ...
    def __getitem__(self, index):
        # Read Sample
        img_path, target, imgname, sentence, topic = self.imgs[index], self.labels[index], self.imgnames[index], self.sentences[index], self.topic[index]
        img = imageio.imread(img_path)
        # Read Image
        if len(img.shape) == 2:
            img = np.stack([img] * 3, 2)
        ...
        if self.transform is not None:
            img = self.transform(img)
        # Read Sentence
        sens = detect_mg_patterns(sentence)#[sen.strip() for sen in sentence.split('\n') if sen.strip()]
        ...
        for sen in sens:
            ...
            examples = read_examples(sen, index)
            features = convert_examples_to_features(
                examples=examples, seq_length=self.query_len, tokenizer=self.tokenizer)
            word_id = features[0].input_ids
            word_mask = features[0].input_mask
            word_ids.append(np.array(word_id, dtype=int))
            word_masks.append(np.array(word_mask, dtype=int))
        ...
        return img, target, word_ids, word_masks#, imgname
