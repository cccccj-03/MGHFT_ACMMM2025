...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, fl):
        mixed_query_layer = self.query(hidden_states)
        mixed_key_layer = self.key(hidden_states)
        mixed_value_layer = self.value(hidden_states)
        text_value_layer = self.value(fl)
        text_key_layer = self.key(fl)

        query_layer = self.transpose_for_scores(mixed_query_layer)  # Multi-head:12*64=768
        key_layer = self.transpose_for_scores(mixed_key_layer)
        value_layer = self.transpose_for_scores(mixed_value_layer)
        key_layer_t = self.transpose_for_scores(text_key_layer)
        value_layer_t = self.transpose_for_scores(text_value_layer)

        attention_scores = torch.matmul(query_layer, key_layer_t.transpose(-1, -2))
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
        attention_probs = self.softmax(attention_scores)
        # weights = attention_probs
        attention_probs = self.attn_dropout(attention_probs)
        v_t =  torch.matmul(attention_probs, value_layer_t)


        attention_scores = torch.matmul(v_t, key_layer.transpose(-1, -2))
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
        attention_probs = self.softmax(attention_scores)
        # weights = attention_probs
        attention_probs = self.attn_dropout(attention_probs)
...
class FuseBlock(nn.Module):
    ...
    def forward(self, x, feat_l):
        ...
        x = self.ffn(x)
        x = x + h
        return x[:,0] #torch.mean(x, dim=1) #`x[:,0]
