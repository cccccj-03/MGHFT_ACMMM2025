...
class MGHFT(nn.Module):
    def __init__(self, args):
        super(MGHFT, self).__init__()
        ...
        # self.pos_embed = nn.Parameter(torch.zeros(1, 1+512, self.hidden_dim))  
        ...
        self.vl_proj = nn.Linear(self.vision_model.embed_dims[3], self.hidden_dim)
        self.l_proj = nn.Linear(self.bert_dim, self.vision_model.embed_dims[3])
    ...
    def _init_weights(self):
        ...
        nn.init.xavier_uniform_(self.vl_proj.weight)
        nn.init.normal_(self.vl_proj.bias, std=1e-6)
    ...
    #     B = image.shape[0]
    #     ## Vision features
    #     fv = self.model.get_image_features(image, output_hidden_states=True) # b, 768
    #     ## Language features
    #     all_encoder_layers, _ = self.text_model(word_id, \
    #         token_type_ids=None, attention_mask=word_mask)
    #     fl = all_encoder_layers[-1]   # [bs, 30, 768]
    #     if not self.tunebert:
    #         fl = fl.detach()
    #     ## Fusion
    #     # x = torch.cat([self.cls_token.expand([B, -1, -1]), self.v_proj(fv), self.l_proj(fl)], dim=1)
    #     x = torch.cat([self.v_proj(fv).unsqueeze(1), self.l_proj(fl)], dim=1)
    #     x = x+self.pos_embed
    ...
    def forward(self, image, word_id=None, word_mask=None, inference=False):
        B = image.shape[0]
        ...
        ## Language features
        if inference:
            feat, loss_cm = self.vision_model(image)
        else:
            for i in range(0, word_id.shape[1]):
                id = word_id[:,i].to(image.device)
                mask = word_mask[:,i].to(image.device)
                all_encoder_layers, _  = self.text_model(id, \
                    token_type_ids=None, attention_mask=mask)
                fl = (all_encoder_layers[-1] + all_encoder_layers[-2]\
                    + all_encoder_layers[-3] + all_encoder_layers[-4])/4  # [bs, 30, 768]
                if not self.tunebert:
                    fl = fl.detach()
                fls.append(self.l_proj(fl.mean(dim=1)))
                # feat_l.append(self.vl_proj(fl.mean(dim=1)))
            ## Vision features
            feat, loss_cm, feat_v = self.vision_model(image, fls)

        feat_v = self.vl_proj(feat_v)
        x = self.v_proj(feat)
        x = x+self.pos_embed
        x = self.fusion_model(x, feat_v)
        ## Prediction
        x = self.head(x)
        return x, loss_cm
